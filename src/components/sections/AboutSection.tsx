              ...
              <Card className="mb-4 shadow-md border-yellow-100 bg-[#fffdf6]">
                <CardContent className="p-6">
                  <div className="flex justify-between items-center mb-4">
                    <h4 className="text-lg font-medium">{isHourlyRate ? 'Hourly Rate' : 'Standard Rate'}</h4>
                    <div className="flex items-center">
                      <span className={`mr-2 text-sm ${!isHourlyRate ? 'font-semibold' : ''}`}>Daily</span>
                      <Switch 
                        checked={isHourlyRate}
                        onCheckedChange={setIsHourlyRate}
                        className={`${isHourlyRate ? 'bg-theme-olive' : 'bg-gray-300'}`}
                      />
                      <span className={`ml-2 text-sm ${isHourlyRate ? 'font-semibold' : ''}`}>Hourly</span>
                    </div>
                  </div>
                  <div className="mb-4">
                    {isHourlyRate ? (
